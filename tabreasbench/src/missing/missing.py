...
def get_questions_and_answers(questions_path: str, data_path: str, scale: str, qtypes) -> Dict[str, pd.DataFrame]:
    ...
    for qtype in qtypes:
        questions[qtype] = {}
        for table in tables:
            df = pd.read_sql(f"SELECT * FROM '{table}' WHERE qtype = ? AND scale = ?", conn, params=(qtype, scale))
            questions[qtype][table] = df
